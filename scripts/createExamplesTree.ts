...
async function traverse(
  ...
  const nodes = await Promise.all(
    entries.map(async (entry) => {
      ...
      const contentHighlighted = await md.renderAsync(`\`\`\`${lang} \n${content}\n\`\`\``);
      const treeNode: TypeTreeNode = { lang, name: entry.name };

      if (contentMap.has(contentHighlighted)) {
        const equalFilePath = contentMap.get(contentHighlighted)!;

        // uncomment to log equal files and the made optimizations
        // const equalFramework = equalFilePath.split('/')[0];
        //
        // console.log(
        //   `\x1b[32m[tree]\x1b[0m ${framework} ${relativePath.split('src/')[1]} === ${equalFramework}`
        // );

        treeNode.content = `linked:${equalFilePath}`;
      } else {
        contentMap.set(contentHighlighted, `${framework}/${relativePath}`);

        treeNode.content = contentHighlighted;
      }
...
function getSize() {
  return ((fs.existsSync(outputFilePath) ? fs.statSync(outputFilePath).size : 0) / 1024).toFixed(2);
}
...
export async function createExamplesTree() {
  ...
  let logMessage = `\x1b[32m[tree]\x1b[0m size \x1b[33m${prevSize}kb\x1b[0m`;

  if (prevSize !== size) logMessage += `-> \x1b[33m${size}kb\x1b[0m`;
