...
:::tip
Всегда используйте `return` с `redirect` для стабильной логики редиректов.
:::

Редиректы из `beforeEnter` наследуют режим истории исходного перехода: обычная цепочка
`router.redirect` завершится через `pushState`, а `router.redirect({ ..., replace: true })` завершится
через `replaceState`. Из-за браузерных ограничений Back / Forward переходы нельзя заблокировать,
поэтому **Reactive Route** принудительно добавляет `replace: true` при редиректах во время браузерного
`popstate`. Это решает множество проблем и циклов вида Back -> запрещенная страница -> push на
разрешенную -> Back -> запрещенная страница. Для эмуляции "блокирования перехода" можно использовать конструкцию

```ts
beforeEnter: ({ redirect, currentState }) => redirect(currentState)
```

Однако в браузерной истории появятся дубликаты History Entry, и при переходе назад-вперед 
на странице не будет видимых изменений. К сожалению, ни один подход не позволяет обойти это
ограничение.

### config.beforeLeave

Эта асинхронная функция вызывается перед уходом с текущего роута, включая браузерную навигацию
Back / Forward `popstate`. Необработанные ошибки приведут к рендеру `internalError` без изменения 
URL в браузере.
