...
Навигация браузера Back/Forward не блокируется. Если `beforeEnter` делает редирект во время
браузерного `popstate`, Reactive Route канонизирует текущую history entry через `replaceState`,
а не добавляет новую entry через `pushState`.

Так не возникает цикл вида Back -> запрещенная страница -> push защищенной страницы -> Back ->
запрещенная страница. Компромисс в том, что Forward может выглядеть как no-op, если две соседние
entry были канонизированы в один и тот же URL.

### config.beforeLeave

Эта асинхронная функция вызывается перед уходом с текущего роута, включая браузерную навигацию
Back/Forward `popstate`. Она не может блокировать или откатить навигацию. Необработанные ошибки
приведут к рендеру `internalError` без изменения URL в браузере.
...
:::tip
Используйте редиректы в `beforeEnter`, чтобы канонизировать запрещенные направления, а не блокировать их.
:::
