...
Асинхронные методы `beforeEnter` позволяют делать редиректы по правилам доступа
и загружать данные в сторы. `beforeLeave` вызывается перед уходом с активного роута
и полезен для cleanup или аналитики. `beforeComponentChange` позволяет проектировать модульные архитектуры
...
Навигация браузера Back/Forward уже меняет текущую history entry к моменту обработки `popstate`,
поэтому Reactive Route не пытается отменить переход. `beforeLeave` все еще вызывается как
side-effect lifecycle, но не может откатить браузерную навигацию.
Если `beforeEnter` делает редирект во время `popstate`, текущая entry канонизируется через
`replaceState`, а не добавляется новая entry через `pushState`.
