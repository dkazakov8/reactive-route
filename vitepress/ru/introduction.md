...
Асинхронные методы `beforeEnter` и `beforeLeave` позволяют контролировать доступ и загружать данные 
в сторы, а `beforeComponentChange` - проектировать модульные архитектуры
...
Библиотека может использоваться с версиями TypeScript `5.0 - 5.6`, но определенные типы при этом 
деградируют. Для полной функциональности требуется TypeScript `>= 5.7`.
...
Список breaking changes, которые могут потребовать изменений в коде.

## v1 -> v2

- В `beforeEnter redirect` больше нельзя передавать `replace`. Он наследуется от 
  исходного перехода: `router.redirect({ ..., replace: true })` завершит всю
  цепочку редиректов через `replaceState`, а обычный `router.redirect(...)` - через `pushState`

- В браузерных Back / Forward `popstate`-переходах `beforeEnter redirect` принудительно использует
  `replaceState`, чтобы избежать бесконечных циклов и сломанного перехода "Назад"

- Из `beforeLeave` удалён `preventRedirect`, потому что он создавал ложное ожидание, что браузерные
  Back / Forward `popstate`-переходы можно заблокировать. Если нужно обработать запрещённый
  переход, используйте `beforeEnter redirect`
